...
import com.ai.agent.application.service.impl.DsTokenPlanServiceImpl;
 ...
 *
 *               POST /api/dashscope-tokenplan/chat        同步对话
 *               POST /api/dashscope-tokenplan/chat/stream  流式对话，SSE 实时推送 chunk
 *
 * @ProjectName: ai-agent
 * @Package: com.ai.agent.starter.controller
 * @ClassName: DsTokenPlanChatController
 * @Author: HUANGcong
...
@Validated
@RestController
@RequestMapping("/api/dashscope-tokenplan")
public class DsTokenPlanChatController {

    private final DsTokenPlanServiceImpl dsTokenPlanService;

    public DsTokenPlanChatController(DsTokenPlanServiceImpl dsTokenPlanService) {
        this.dsTokenPlanService = dsTokenPlanService;
    }

    @PostMapping("/chat")
    public Result<LlmResponseVO> chat(@Valid @RequestBody LlmRequestVO req) {
        log.info("[DashscopeTokenPlan-chat] 开始处理, req={}", req);
        try {
            LlmResponse response = dsTokenPlanService.chat(toServiceRequest(req));
            log.info("[DashscopeTokenPlan-chat] 处理完成, response={}", response);
            return Result.success(toVO(response));
        } catch (BizException e) {
            throw e;
        } catch (Exception e) {
            log.error("[DashscopeTokenPlan-chat] 系统异常", e);
    ...
    public SseEmitter chatStream(@Valid @RequestBody LlmRequestVO req) {
        log.info("[DashscopeTokenPlan-stream] 开始处理, req={}", req);
        SseEmitter emitter = new SseEmitter(0L);
        dsTokenPlanService.chatStream(toServiceRequest(req), buildSseConsumer(emitter, req.getModelCode()));
        return emitter;
    }
        ...
        return chunk -> {
            if (chunk == null) {
                try {
                    ...
                    log.warn("[DashscopeTokenPlan-stream] 发送 done 事件失败, model={}", tag);
                }
                emitter.complete();
                ...
                try {
                    ...
                    log.warn("[DashscopeTokenPlan-stream] 客户端已断开, model={}", tag);
