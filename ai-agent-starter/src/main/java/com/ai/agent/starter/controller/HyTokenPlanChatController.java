...
import com.ai.agent.application.service.impl.HyTokenPlanServiceImpl;
 ...
 *
 *               POST /api/hy-tokenplan/chat        同步对话
 *               POST /api/hy-tokenplan/chat/stream  流式对话，SSE 实时推送 chunk
 *
 * @ProjectName: ai-agent
 * @Package: com.ai.agent.starter.controller
 * @ClassName: HyTokenPlanChatController
 * @Author: HUANGcong
...
@Validated
@RestController
@RequestMapping("/api/hy-tokenplan")
public class HyTokenPlanChatController {

    private final HyTokenPlanServiceImpl hyTokenPlanService;

    public HyTokenPlanChatController(HyTokenPlanServiceImpl hyTokenPlanService) {
        this.hyTokenPlanService = hyTokenPlanService;
    }

    @PostMapping("/chat")
    public Result<LlmResponseVO> chat(@Valid @RequestBody LlmRequestVO req) {
        log.info("[HunyuanTokenPlan-chat] 开始处理, req={}", req);
        try {
            LlmResponse response = hyTokenPlanService.chat(toServiceRequest(req));
            log.info("[HunyuanTokenPlan-chat] 处理完成, response={}", response);
            return Result.success(toVO(response));
        } catch (BizException e) {
            throw e;
        } catch (Exception e) {
            log.error("[HunyuanTokenPlan-chat] 系统异常", e);
    ...
    public SseEmitter chatStream(@Valid @RequestBody LlmRequestVO req) {
        log.info("[HunyuanTokenPlan-stream] 开始处理, req={}", req);
        SseEmitter emitter = new SseEmitter(0L);
        hyTokenPlanService.chatStream(toServiceRequest(req), buildSseConsumer(emitter, req.getModelCode()));
        return emitter;
    }
        ...
        return chunk -> {
            if (chunk == null) {
                try {
                    ...
                    log.warn("[HunyuanTokenPlan-stream] 发送 done 事件失败, model={}", tag);
                }
                emitter.complete();
                ...
                try {
                    ...
                    log.warn("[HunyuanTokenPlan-stream] 客户端已断开, model={}", tag);
