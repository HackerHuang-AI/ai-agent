...
import com.ai.agent.application.service.impl.AnthropicServiceImpl;
...
/**
 * @Description: Anthropic Claude 平台对话接口
 *
 *               POST /api/anthropic/chat        同步对话
 *               POST /api/anthropic/chat/stream  流式对话，SSE 实时推送 chunk
 *
 * @ProjectName: ai-agent
 * @Package: com.ai.agent.starter.controller
 * @ClassName: AnthropicChatController
 * @Author: HUANGcong
...
public class AnthropicChatController {
    ...
    public AnthropicChatController(AnthropicServiceImpl anthropicService) {
        this.anthropicService = anthropicService;
    }

    @PostMapping("/chat")
    public Result<LlmResponseVO> chat(@Valid @RequestBody LlmRequestVO req) {
        log.info("[Anthropic-chat] 开始处理, req={}", req);
        try {
            LlmResponse response = anthropicService.chat(toServiceRequest(req));
            log.info("[Anthropic-chat] 处理完成, response={}", response);
            return Result.success(toVO(response));
        } catch (BizException e) {
            throw e;
        } catch (Exception e) {
            log.error("[Anthropic-chat] 系统异常", e);
    ...
    public SseEmitter chatStream(@Valid @RequestBody LlmRequestVO req) {
        log.info("[Anthropic-stream] 开始处理, req={}", req);
        SseEmitter emitter = new SseEmitter(0L);
        anthropicService.chatStream(toServiceRequest(req), buildSseConsumer(emitter, req.getModelCode()));
        return emitter;
    }
        ...
        return chunk -> {
            if (chunk == null) {
                try {
                    ...
                    log.warn("[Anthropic-stream] 发送 done 事件失败, model={}", tag);
                }
                emitter.complete();
                ...
                try {
                    ...
                    log.warn("[Anthropic-stream] 客户端已断开, model={}", tag);
