 ...
 *
 *               POST /api/openai/chat        同步对话
 *               POST /api/openai/chat/stream  流式对话，SSE 实时推送 chunk
 *
...
public class OpenAiChatController {
    ...
    public OpenAiChatController(@Qualifier("openaiServiceImpl") LlmService llmService) {
        ...
        return chunk -> {
            if (chunk == null) {
                try {
                    ...
                    log.warn("[OpenAI-stream] 发送 done 事件失败, model={}", tag);
                }
                emitter.complete();
                ...
                try {
                    ...
                    log.warn("[OpenAI-stream] 客户端已断开, model={}", tag);
