 ...
 *
 *               POST /api/moonshot/chat        同步对话
 *               POST /api/moonshot/chat/stream  流式对话，SSE 实时推送 chunk
 *
 * @ProjectName: ai-agent
 * @Package: com.ai.agent.starter.controller
 * @ClassName: MoonshotChatController
 * @Author: HUANGcong
...
public class MoonshotChatController {
    ...
    public Result<LlmResponseVO> chat(@Valid @RequestBody LlmRequestVO req) {
        ...
        try {
            ...
            log.error("[Moonshot-chat] 系统异常", e);
        ...
        return chunk -> {
            if (chunk == null) {
                try {
                    ...
                    log.warn("[Moonshot-stream] 发送 done 事件失败, model={}", tag);
                }
                emitter.complete();
            } else {
                try {
                    ...
                    log.warn("[Moonshot-stream] 客户端已断开, model={}", tag);
