 ...
 * @Package: com.ai.agent.starter.controller
 * @ClassName: QianfanChatController
 * @Author: HUANGcong
...
public class QianfanChatController {
    ...
    public Result<LlmResponseVO> chat(@Valid @RequestBody LlmRequestVO req) {
        log.info("[Qianfan-chat] 开始处理, req={}", req);
        try {
            LlmResponse response = qianfanService.chat(toServiceRequest(req));
            log.info("[Qianfan-chat] 处理完成, response={}", response);
            return Result.success(toVO(response));
        } catch (BizException e) {
            throw e;
        } catch (Exception e) {
            log.error("[Qianfan-chat] 系统异常", e);
    ...
    public SseEmitter chatStream(@Valid @RequestBody LlmRequestVO req) {
        log.info("[Qianfan-stream] 开始处理, req={}", req);
        SseEmitter emitter = new SseEmitter(0L);
        qianfanService.chatStream(toServiceRequest(req), buildSseConsumer(emitter, req.getModelCode()));
        return emitter;
    }
        ...
        return chunk -> {
            if (chunk == null) {
                try {
                    ...
                    log.warn("[Qianfan-stream] 发送 done 事件失败, model={}", tag);
                }
                emitter.complete();
                ...
                try {
                    ...
                    log.warn("[Qianfan-stream] 客户端已断开, model={}", tag);
