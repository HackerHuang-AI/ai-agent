...
public class ZhipuServiceImpl implements LlmService {
    ...
    public LlmResponse chat(LlmRequest request) {
            ...
            try {
                ...
                try (Response response = HTTP_CLIENT.newCall(okRequest).execute()) {
                    ...
                    if (!response.isSuccessful()) {
                        ...
                        log.warn("[Zhipu-chat] HTTP {}，尝试 {}/{}, platformError={}", response.code(), attempt, MAX_ATTEMPTS, platformErr);
                        lastException = new IOException("HTTP " + response.code());
                    } else if (responseBody.isEmpty()) {
                        log.warn("[Zhipu-chat] 响应体为空，尝试 {}/{}", attempt, MAX_ATTEMPTS);
                ...
                log.warn("[Zhipu-chat] IO 异常，尝试 {}/{}: {}", attempt, MAX_ATTEMPTS, e.getMessage());
                lastException = e;
            }
        ...
        log.error("[Zhipu-chat] 重试 {} 次后仍失败", MAX_ATTEMPTS, lastException);
