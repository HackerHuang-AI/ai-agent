...
public class DeepseekServiceImpl implements LlmService {
    ...
    private void fillDefaults(LlmRequest request) {
        ...
        if (StringUtils.isBlank(request.getApiKey())
            ...
            cfg = nacosConfig.getObject(NacosDataIdEnum.AI_AGENT_DEEPSEEK, "chat", DeepseekBO.class);
        }
        if (StringUtils.isBlank(request.getApiKey()))
    ...
    private String buildRequestBody(LlmRequest request, boolean stream) {
        ...
        // 支持的透传参数包括：response_format、stop、logprobs、top_logprobs、user_id 等
        if (extra != null) {
            extra.forEach((k, v) -> {
    ...
    private void flushToolCalls(Map<Integer, String[]> toolCallsMap, String modelCode, Consumer<String> chunkConsumer) {
        ...
        try {
            ...
            log.warn("[Deepseek-stream] tool_calls 序列化失败，跳过, model={}", modelCode, e);
        }
    }
}
