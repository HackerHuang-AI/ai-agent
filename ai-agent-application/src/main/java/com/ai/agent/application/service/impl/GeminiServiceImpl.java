...
public class GeminiServiceImpl implements LlmService {
    ...
    public GeminiServiceImpl(@Qualifier("geminiStreamExecutor") ExecutorService streamExecutor,
    ...
    public LlmResponse chat(LlmRequest request) {
        ...
        log.info("[Gemini-chat] 开始调用, model={}, endpoint={}", request.getModelCode(), request.getEndpoint());
        ...
        LlmResponse result = AppRetryUtil.retry(() -> {
            ...
            try (Response response = okHttpConfig.getClientByPlatform("gemini").newCall(okRequest).execute()) {
                    String responseBody = response.body() != null ? response.body().string() : "";
                    if (!response.isSuccessful()) {
                        String platformErr = extractErrorMessage(responseBody);
                        log.error("[Gemini-chat] HTTP {} 失败, platformError={}", response.code(), platformErr);
                        throwByHttpCode(response.code(), platformErr);
                    }
                    if (responseBody.isEmpty()) {
                        log.error("[Gemini-chat] 响应体为空");
    ...
    public void chatStream(LlmRequest request, Consumer<String> chunkConsumer) {
        ...
        log.info("[Gemini-stream] 开始调用, model={}, endpoint={}", request.getModelCode(), request.getEndpoint());
        ...
        try {
            streamExecutor.submit(() -> {
                ...
                try {
                    ...
                    Response response = AppRetryUtil.retryForStream(() -> {
                        Response resp = okHttpConfig.getClientByPlatform("gemini").newCall(okRequest).execute();
                        if (!resp.isSuccessful()) {
                            String errBody = resp.body() != null ? resp.body().string() : "";
                            String platformMsg = extractErrorMessage(errBody);
                            log.error("[Gemini-stream] HTTP {} 失败, platformError={}", resp.code(), platformMsg);
                            resp.close();
                    ...
                    log.error("[Gemini-stream] 未预期异常", e);
                    chunkConsumer.accept("[ERROR]");
                } finally {
                    MDC.clear();
                }
            ...
            log.error("[Gemini-stream] 线程池已满，拒绝请求", e);
            chunkConsumer.accept("[ERROR]");
        }
    }
    ...
    private void fillDefaults(LlmRequest request) {
        ...
        if (StringUtils.isBlank(request.getApiKey())
            ...
            cfg = NacosConfigUtil.getObject(NacosDataIdEnum.AI_AGENT_GEMINI, "chat", GeminiBO.class);
        }
        if (StringUtils.isBlank(request.getApiKey()))
        ...
        if (StringUtils.isBlank(request.getApiKey())) {
            log.error("[Gemini] apiKey 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.LLM_API_KEY_NOT_FOUND);
        }
        if (StringUtils.isBlank(request.getEndpoint())) {
            log.error("[Gemini] endpoint 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.PARAM_ILLEGAL);
        }
        if (StringUtils.isBlank(request.getModelCode())) {
            log.error("[Gemini] modelCode 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.PARAM_ILLEGAL);
        }
    }
    ...
    private LlmResponse parseResponse(String responseJson, String modelCode) {
        try {
            ...
            log.error("[Gemini-chat] 响应解析失败", e);
    ...
    private void parseStreamResponse(ResponseBody responseBody, String modelCode, Consumer<String> chunkConsumer) {
        try (BufferedReader reader = new BufferedReader(
            ...
            log.error("[Gemini-stream] 流式响应解析失败, model={}", modelCode, e);
            throw new BizException(ErrorCodeEnum.LLM_RESPONSE_PARSE_FAILED);
        }
    }
