...
import com.ai.agent.application.bo.DsTokenPlanBO;
 ...
 * @Package: com.ai.agent.application.service.impl
 * @ClassName: DsTokenPlanServiceImpl
 * @Author: HUANGcong
...
@Slf4j
@Service
public class DsTokenPlanServiceImpl implements LlmService {
    ...
    public LlmResponse chat(LlmRequest request) {
        ...
        log.info("[DashscopeTokenPlan-chat] 开始调用, model={}, endpoint={}", request.getModelCode(), request.getEndpoint());
        ...
        LlmResponse result = RetryUtil.retry(() -> {
            ...
            try (Response response = okHttpConfig.getLlmClient("ds_tokenplan").newCall(okRequest).execute()) {
                    ...
                    if (!response.isSuccessful()) {
                        ...
                        log.error("[DashscopeTokenPlan-chat] HTTP {} 失败, platformError={}", response.code(), platformErr);
    ...
    public void chatStream(LlmRequest request, Consumer<String> chunkConsumer) {
        ...
        log.info("[DashscopeTokenPlan-stream] 开始调用, model={}, endpoint={}", request.getModelCode(), request.getEndpoint());
        ...
        try {
            streamExecutor.submit(() -> {
                ...
                try {
                    ...
                    log.error("[DashscopeTokenPlan-stream] 业务异常", e);
                    chunkConsumer.accept("[ERROR]");
                } catch (IOException e) {
                    log.error("[DashscopeTokenPlan-stream] IO 异常", e);
                    chunkConsumer.accept("[ERROR]");
                } finally {
                    MDC.clear();
                }
            ...
            log.error("[DashscopeTokenPlan-stream] 线程池已满，拒绝请求", e);
            chunkConsumer.accept("[ERROR]");
        }
    }
    ...
    private void fillDefaults(LlmRequest request) {
        DsTokenPlanBO cfg = null;
        if (StringUtils.isBlank(request.getApiKey())
                || StringUtils.isBlank(request.getEndpoint())
                || StringUtils.isBlank(request.getModelCode())) {
            cfg = NacosConfigUtil.getObject(NacosDataIdEnum.AI_AGENT_DASHSCOPE_TOKENPLAN, "chat", DsTokenPlanBO.class);
        }
        if (StringUtils.isBlank(request.getApiKey()))
        ...
        if (StringUtils.isBlank(request.getApiKey())) {
            log.error("[灵积TokenPlan] apiKey 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.LLM_API_KEY_NOT_FOUND);
        }
        if (StringUtils.isBlank(request.getEndpoint())) {
            log.error("[灵积TokenPlan] endpoint 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.PARAM_ILLEGAL);
        }
        if (StringUtils.isBlank(request.getModelCode())) {
            log.error("[灵积TokenPlan] modelCode 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.PARAM_ILLEGAL);
        }
    }
    ...
    private LlmResponse parseResponse(String responseJson, String modelCode) {
        try {
            ...
            log.error("[DashscopeTokenPlan-chat] 响应解析失败", e);
    ...
    private void parseStreamResponse(ResponseBody responseBody, String modelCode, Consumer<String> chunkConsumer) {
        try (BufferedReader reader = new BufferedReader(
            ...
            log.error("[DashscopeTokenPlan-stream] 流式响应解析失败, model={}", modelCode, e);
            throw new BizException(ErrorCodeEnum.LLM_RESPONSE_PARSE_FAILED);
        }
    }
