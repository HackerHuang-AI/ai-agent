...
@Slf4j
@Service
public class MimoServiceImpl implements LlmService {
    ...
    public MimoServiceImpl(@Qualifier("mimoStreamExecutor") ExecutorService streamExecutor,
    ...
    public LlmResponse chat(LlmRequest request) {
        ...
        log.info("[MiMo-chat] 开始调用, model={}, endpoint={}", request.getModelCode(), request.getEndpoint());
        ...
        LlmResponse result = AppRetryUtil.retry(() -> {
            ...
            try (Response response = okHttpConfig.getClientByPlatform(OkHttpConfigEnum.MIMO).newCall(okRequest).execute()) {
                    String responseBody = response.body() != null ? response.body().string() : "";
                    if (!response.isSuccessful()) {
                        String platformErr = extractErrorMessage(responseBody);
                        log.error("[MiMo-chat] HTTP {} 失败, platformError={}", response.code(), platformErr);
                        throwByHttpCode(response.code(), platformErr);
                    }
                    if (responseBody.isEmpty()) {
                        log.error("[MiMo-chat] 响应体为空");
                        throw new BizException(ErrorCodeEnum.LLM_CALL_FAILED);
                    }
                    return parseResponse(responseBody, request.getModelCode());
            }
        }, retryConfig.getRetryParam(RetryConfigEnum.MIMO));
    ...
    public void chatStream(LlmRequest request, Consumer<String> chunkConsumer) {
        ...
        log.info("[MiMo-stream] 开始调用, model={}, endpoint={}", request.getModelCode(), request.getEndpoint());
        ...
        try {
            streamExecutor.submit(() -> {
                ...
                try {
                    ...
                    Response response = AppRetryUtil.retryForStream(() -> {
                        Response resp = okHttpConfig.getClientByPlatform(OkHttpConfigEnum.MIMO).newCall(okRequest).execute();
                        if (!resp.isSuccessful()) {
                            String errBody = resp.body() != null ? resp.body().string() : "";
                            String platformMsg = extractErrorMessage(errBody);
                            log.error("[Mimo-stream] HTTP {} 失败, platformError={}", resp.code(), platformMsg);
                            resp.close();
            ...
            log.error("[MiMo-stream] 线程池已满，拒绝请求", e);
            chunkConsumer.accept("[ERROR]");
        }
    }
    ...
    private void fillDefaults(LlmRequest request) {
        ...
        if (StringUtils.isBlank(request.getApiKey())
            ...
            cfg = nacosConfig.getObject(NacosDataIdEnum.AI_AGENT_MIMO, "chat", MimoBO.class);
        }
        if (StringUtils.isBlank(request.getApiKey()))
        ...
        if (StringUtils.isBlank(request.getApiKey())) {
            log.error("[MiMo] apiKey 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.LLM_API_KEY_NOT_FOUND);
        }
        if (StringUtils.isBlank(request.getEndpoint())) {
            log.error("[MiMo] endpoint 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.PARAM_ILLEGAL);
        }
        if (StringUtils.isBlank(request.getModelCode())) {
            log.error("[MiMo] modelCode 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.PARAM_ILLEGAL);
        }
    }
    ...
    private void parseStreamResponse(ResponseBody responseBody, String modelCode, Consumer<String> chunkConsumer) {
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            log.error("[MiMo-stream] 流式响应解析失败, model={}", modelCode, e);
            throw new BizException(ErrorCodeEnum.LLM_RESPONSE_PARSE_FAILED);
        }
    }
    ...
    private void flushToolCalls(Map<Integer, String[]> toolCallsMap, String modelCode, Consumer<String> chunkConsumer) {
        ...
        try {
            ...
            log.warn("[MiMo-stream] tool_calls 序列化失败，跳过, model={}", modelCode, e);
        }
    }
