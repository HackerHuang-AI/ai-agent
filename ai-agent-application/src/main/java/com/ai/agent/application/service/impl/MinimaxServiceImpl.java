    ...
    public LlmResponse chat(LlmRequest request) {
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                try (Response response = HTTP_CLIENT.newCall(okRequest).execute()) {
                    ...
                    if (!response.isSuccessful()) {
                        ...
                        if (response.code() >= 400 && response.code() < 500) {
                            log.error("[Minimax-chat] HTTP {}，不可重试, platformError={}", response.code(), platformErr);
                            throw new BizException(ErrorCodeEnum.LLM_CALL_FAILED, platformErr);
                        }
                        log.warn("[Minimax-chat] HTTP {}，尝试 {}/{}", response.code(), attempt, MAX_ATTEMPTS);
                        lastException = new IOException("HTTP " + response.code());
                    } else if (responseBody.isEmpty()) {
                        log.warn("[Minimax-chat] 响应体为空，尝试 {}/{}", attempt, MAX_ATTEMPTS);
                        lastException = new IOException("响应体为空");
                    } else {
                        // Minimax 特有：HTTP 200 仍需检查业务状态码
                ...
                log.warn("[Minimax-chat] IO 异常，尝试 {}/{}: {}", attempt, MAX_ATTEMPTS, e.getMessage());
                lastException = e;
            }
        ...
        log.error("[Minimax-chat] 重试 {} 次后仍失败", MAX_ATTEMPTS, lastException);
