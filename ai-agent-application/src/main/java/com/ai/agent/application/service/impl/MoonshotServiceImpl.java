...
import com.ai.agent.application.enums.http.MoonshotHttpCode;
...
public class MoonshotServiceImpl implements LlmService {
    ...
    private void throwByHttpCode(int httpCode, String platformMsg) {
        ...
        if (httpCode == MoonshotHttpCode.UNAUTHORIZED.getCode()) {
            errorCode = ErrorCodeEnum.LLM_AUTH_FAILED;
        } else if (httpCode == MoonshotHttpCode.INSUFFICIENT_FUNDS.getCode()) {
            errorCode = ErrorCodeEnum.LLM_INSUFFICIENT_BALANCE;
        } else if (httpCode == MoonshotHttpCode.BAD_REQUEST.getCode()
                || httpCode == MoonshotHttpCode.UNPROCESSABLE.getCode()) {
            errorCode = ErrorCodeEnum.PARAM_ILLEGAL;
        } else if (httpCode == MoonshotHttpCode.RATE_LIMIT.getCode()) {
