...
import com.ai.agent.application.enums.http.MoonshotHttpCodeEnum;
...
public class MoonshotServiceImpl implements LlmService {
    ...
    public LlmResponse chat(LlmRequest request) {
        ...
        LlmResponse result = AppRetryUtil.retry(() -> {
            ...
            try (Response response = okHttpConfig.getClientByPlatform(OkHttpConfigEnum.MOONSHOT).newCall(okRequest).execute()) {
                    String responseBody = response.body() != null ? response.body().string() : "";
                    if (!response.isSuccessful()) {
                        String platformErr = extractErrorMessage(responseBody);
                        log.error("[Moonshot-chat] HTTP {} 失败, platformError={}", response.code(), platformErr);
                        throwByHttpCode(response.code(), platformErr);
                    }
                    if (responseBody.isEmpty()) {
                        log.error("[Moonshot-chat] 响应体为空");
                        throw new BizException(ErrorCodeEnum.LLM_CALL_FAILED);
                    }
                    return parseResponse(responseBody, request.getModelCode());
            }
        }, retryConfig.getRetryParam(RetryConfigEnum.MOONSHOT));
    ...
    public void chatStream(LlmRequest request, Consumer<String> chunkConsumer) {
        ...
        try {
            streamExecutor.submit(() -> {
                ...
                try {
                    ...
                    Response response = AppRetryUtil.retryForStream(() -> {
                        Response resp = okHttpConfig.getClientByPlatform(OkHttpConfigEnum.MOONSHOT).newCall(okRequest).execute();
                        if (!resp.isSuccessful()) {
                            String errBody = resp.body() != null ? resp.body().string() : "";
                            String platformMsg = extractErrorMessage(errBody);
                            log.error("[Moonshot-stream] HTTP {} 失败, platformError={}", resp.code(), platformMsg);
                            resp.close();
            ...
            log.error("[Moonshot-stream] 线程池已满，拒绝请求", e);
            chunkConsumer.accept("[ERROR]");
        }
    }
    ...
    private void fillDefaults(LlmRequest request) {
        ...
        if (StringUtils.isBlank(request.getApiKey())
            ...
            cfg = nacosConfig.getObject(NacosDataIdEnum.AI_AGENT_MOONSHOT, "chat", MoonshotBO.class);
        }
        if (StringUtils.isBlank(request.getApiKey()))
    ...
    private String buildRequestBody(LlmRequest request, boolean stream) {
        ...
        // Moonshot 官方文档无 frequency_penalty 参数，不注入
    ...
    private void flushToolCalls(Map<Integer, String[]> toolCallsMap, String modelCode, Consumer<String> chunkConsumer) {
        ...
        try {
            ...
            log.warn("[Moonshot-stream] tool_calls 序列化失败，跳过, model={}", modelCode, e);
        }
    }
    ...
    private void throwByHttpCode(int httpCode, String platformMsg) {
        ...
        if (httpCode == MoonshotHttpCodeEnum.UNAUTHORIZED.getCode()) {
            errorCode = ErrorCodeEnum.LLM_AUTH_FAILED;
        } else if (httpCode == MoonshotHttpCodeEnum.INSUFFICIENT_FUNDS.getCode()) {
            errorCode = ErrorCodeEnum.LLM_INSUFFICIENT_BALANCE;
        } else if (httpCode == MoonshotHttpCodeEnum.BAD_REQUEST.getCode()
                || httpCode == MoonshotHttpCodeEnum.UNPROCESSABLE.getCode()) {
            errorCode = ErrorCodeEnum.PARAM_ILLEGAL;
        } else if (httpCode == MoonshotHttpCodeEnum.RATE_LIMIT.getCode()) {
