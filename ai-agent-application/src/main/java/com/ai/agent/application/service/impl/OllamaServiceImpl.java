...
public class OllamaServiceImpl implements LlmService {
    ...
    private void fillDefaults(LlmRequest request) {
        ...
        if (StringUtils.isBlank(request.getEndpoint())
            ...
            cfg = nacosConfig.getObject(NacosDataIdEnum.AI_AGENT_OLLAMA, "chat", OllamaBO.class);
        }
    ...
    /**
     * 构造 tool_choice 字段：auto/none 原样透传字符串；其余值视为指定工具名，转为 OpenAI 兼容协议要求的对象结构。注：需本地加载支持 function calling 的模型。
     */
    ...
    private void flushToolCalls(Map<Integer, String[]> toolCallsMap, String modelCode, Consumer<String> chunkConsumer) {
        ...
        try {
            ...
            log.warn("[Ollama-stream] tool_calls 序列化失败，跳过, model={}", modelCode, e);
        }
    }
