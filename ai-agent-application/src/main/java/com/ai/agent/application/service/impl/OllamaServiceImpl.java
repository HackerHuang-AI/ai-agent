    ...
    public LlmResponse chat(LlmRequest request) {
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                try (Response response = okHttpConfig.getLlmClient().newCall(okRequest).execute()) {
                    ...
                    if (!response.isSuccessful()) {
                        ...
                        if (response.code() >= 400 && response.code() < 500) {
                            log.error("[Ollama-chat] HTTP {}，不可重试, platformError={}", response.code(), platformErr);
                            throw new BizException(ErrorCodeEnum.LLM_CALL_FAILED, platformErr);
                        }
                        log.warn("[Ollama-chat] HTTP {}，尝试 {}/{}, platformError={}", response.code(), attempt, MAX_ATTEMPTS, platformErr);
                        lastException = new IOException("HTTP " + response.code());
                    } else if (responseBody.isEmpty()) {
                        log.warn("[Ollama-chat] 响应体为空，尝试 {}/{}", attempt, MAX_ATTEMPTS);
                ...
                log.warn("[Ollama-chat] IO 异常，尝试 {}/{}: {}", attempt, MAX_ATTEMPTS, e.getMessage());
                lastException = e;
            }
        ...
        log.error("[Ollama-chat] 重试 {} 次后仍失败", MAX_ATTEMPTS, lastException);
