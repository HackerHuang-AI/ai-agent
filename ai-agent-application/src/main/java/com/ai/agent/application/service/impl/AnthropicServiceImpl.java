    ...
    public LlmResponse chat(LlmRequest request) {
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                try (Response response = HTTP_CLIENT.newCall(okRequest).execute()) {
                    ...
                    if (!response.isSuccessful()) {
                        ...
                        if (response.code() >= 400 && response.code() < 500) {
                            log.error("[Anthropic-chat] HTTP {}，不可重试, error={}", response.code(), platformErr);
                            throw new BizException(ErrorCodeEnum.LLM_CALL_FAILED, platformErr);
                        }
                        log.warn("[Anthropic-chat] HTTP {}，尝试 {}/{}", response.code(), attempt, MAX_ATTEMPTS);
                        lastException = new IOException("HTTP " + response.code());
                    } else if (responseBody.isEmpty()) {
                        log.warn("[Anthropic-chat] 响应体为空，尝试 {}/{}", attempt, MAX_ATTEMPTS);
                ...
                log.warn("[Anthropic-chat] IO 异常，尝试 {}/{}: {}", attempt, MAX_ATTEMPTS, e.getMessage());
                lastException = e;
            }
            if (attempt < MAX_ATTEMPTS) sleepQuietly(BASE_DELAY_MS * attempt);
        }
        log.error("[Anthropic-chat] 重试 {} 次后仍失败", MAX_ATTEMPTS, lastException);
