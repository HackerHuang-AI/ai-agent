            ...
            try (Response response = okHttpConfig.getLlmClient("hy_tokenplan").newCall(okRequest).execute()) {
                    ...
                    if (!response.isSuccessful()) {
                        ...
                        if (response.code() >= 400 && response.code() < 500) {
                            log.error("[HunyuanTokenPlan-chat] HTTP {}，不可重试, platformError={}", response.code(), platformErr);
                            throw new BizException(ErrorCodeEnum.LLM_CALL_FAILED, platformErr);
                        }
                        log.warn("[HunyuanTokenPlan-chat] HTTP {} 失败, platformError={}", response.code(), platformErr);
                        throw new BizException(ErrorCodeEnum.LLM_CALL_FAILED, platformErr);
                    }
