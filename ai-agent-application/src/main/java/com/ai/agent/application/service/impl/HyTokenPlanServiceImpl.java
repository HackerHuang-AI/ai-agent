...
public class HyTokenPlanServiceImpl implements LlmService {
    ...
    public LlmResponse chat(LlmRequest request) {
            ...
            try {
                ...
                try (Response response = okHttpConfig.getLlmClient().newCall(okRequest).execute()) {
                    ...
                    if (!response.isSuccessful()) {
                        ...
                        log.warn("[HunyuanTokenPlan-chat] HTTP {}，尝试 {}/{}, platformError={}", response.code(), attempt, MAX_ATTEMPTS, platformErr);
                        lastException = new IOException("HTTP " + response.code());
                    } else if (responseBody.isEmpty()) {
                        log.warn("[HunyuanTokenPlan-chat] 响应体为空，尝试 {}/{}", attempt, MAX_ATTEMPTS);
                ...
                log.warn("[HunyuanTokenPlan-chat] IO 异常，尝试 {}/{}: {}", attempt, MAX_ATTEMPTS, e.getMessage());
                lastException = e;
            }
        ...
        log.error("[HunyuanTokenPlan-chat] 重试 {} 次后仍失败", MAX_ATTEMPTS, lastException);
