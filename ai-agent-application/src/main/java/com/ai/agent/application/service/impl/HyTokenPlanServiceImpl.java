...
import com.ai.agent.application.bo.HyTokenPlanBO;
 ...
 * @Package: com.ai.agent.application.service.impl
 * @ClassName: HyTokenPlanServiceImpl
 * @Author: HUANGcong
...
@Slf4j
@Service
public class HyTokenPlanServiceImpl implements LlmService {
    ...
    public HyTokenPlanServiceImpl(@Qualifier("hyTokenPlanStreamExecutor") ExecutorService streamExecutor,
    ...
    public LlmResponse chat(LlmRequest request) {
        ...
        log.info("[HunyuanTokenPlan-chat] 开始调用, model={}, endpoint={}", request.getModelCode(), request.getEndpoint());
        ...
        LlmResponse result = RetryUtil.retry(() -> {
            ...
            try (Response response = okHttpConfig.getLlmClient("hy_tokenplan").newCall(okRequest).execute()) {
                    String responseBody = response.body() != null ? response.body().string() : "";
                    if (!response.isSuccessful()) {
                        String platformErr = extractErrorMessage(responseBody);
                        log.error("[HunyuanTokenPlan-chat] HTTP {} 失败, platformError={}", response.code(), platformErr);
                        throwByHttpCode(response.code(), platformErr);
                    }
                    if (responseBody.isEmpty()) {
                        log.error("[HunyuanTokenPlan-chat] 响应体为空");
        ...
        }, retryConfig.getRetryParam("hy_tokenplan"));
        if (result == null) throw new BizException(ErrorCodeEnum.LLM_CALL_FAILED);
        log.info("[HunyuanTokenPlan-chat] 调用成功, model={}, inputTokens={}, outputTokens={}, costMs={}",
    ...
    public void chatStream(LlmRequest request, Consumer<String> chunkConsumer) {
        ...
        log.info("[HunyuanTokenPlan-stream] 开始调用, model={}, endpoint={}", request.getModelCode(), request.getEndpoint());
        ...
        try {
                ...
                try {
                    ...
                    try (Response response = okHttpConfig.getLlmClient("hy_tokenplan").newCall(okRequest).execute()) {
                        if (!response.isSuccessful() || response.body() == null) {
                            ...
                            log.error("[HunyuanTokenPlan-stream] HTTP 失败, code={}, platformError={}", response.code(), extractErrorMessage(errBody));
                    ...
                    log.error("[HunyuanTokenPlan-stream] 业务异常", e);
                    chunkConsumer.accept("[ERROR]");
                } catch (IOException e) {
                    log.error("[HunyuanTokenPlan-stream] IO 异常", e);
                    chunkConsumer.accept("[ERROR]");
                } finally {
                    MDC.clear();
                }
            ...
            log.error("[HunyuanTokenPlan-stream] 线程池已满，拒绝请求", e);
            chunkConsumer.accept("[ERROR]");
        }
    }

    /**
     * 腾讯混元 TokenPlan 当前接入版本为纯文本模型，不具备图片理解能力，暂不支持多模态，返回 null。
     */
    @Override
    public LlmResponse multimodalChat(LlmRequest request) {
        log.warn("[HyTokenPlan] 暂不支持多模态：腾讯混元 TokenPlan 当前接入版本为纯文本模型，不具备图片理解能力");
        return null;
    }
    ...
    private void fillDefaults(LlmRequest request) {
        ...
        if (StringUtils.isBlank(request.getApiKey())) {
            log.error("[混元TokenPlan] apiKey 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.LLM_API_KEY_NOT_FOUND);
        }
        if (StringUtils.isBlank(request.getEndpoint())) {
            log.error("[混元TokenPlan] endpoint 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.PARAM_ILLEGAL);
        }
        if (StringUtils.isBlank(request.getModelCode())) {
            log.error("[混元TokenPlan] modelCode 未配置，入参和 Nacos 均为空");
            throw new BizException(ErrorCodeEnum.PARAM_ILLEGAL);
        }
    }
    ...
    private LlmResponse parseResponse(String responseJson, String modelCode) {
        try {
            ...
            log.error("[HunyuanTokenPlan-chat] 响应解析失败", e);
    ...
    private void parseStreamResponse(ResponseBody responseBody, String modelCode, Consumer<String> chunkConsumer) {
        try (BufferedReader reader = new BufferedReader(
            ...
            log.error("[HunyuanTokenPlan-stream] 流式响应解析失败, model={}", modelCode, e);
            throw new BizException(ErrorCodeEnum.LLM_RESPONSE_PARSE_FAILED);
        }
    }
